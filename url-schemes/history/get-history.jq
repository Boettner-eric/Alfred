...
# Merges history/history.json entries with their parent in urls.json.
...
[
  ...
  | (
      [$url_list[]
        | select(.scheme as $scheme | $url | startswith($scheme))
        | . as $entry
        | (($url | ltrimstr($entry.scheme)) | path_and_query) as $parsed
        | $entry.paths[] | select(.path == $parsed.path)
        | . as $path_obj
        | ($parsed.query | query_to_params) as $actual_params
        | ($entry.scheme + $path_obj.path + (if $actual_params | length > 0 then "?" + ($actual_params | to_entries | map("\(.key)=\(.value)") | join("&")) else "" end)) as $arg
        | {
            count: $count,
            item: {
              title: ($entry.name + " - " + $path_obj.title),
              subtitle: (
                "open " + $arg + " · used " + ($count | tostring) + " time" + (if $count == 1 then "" else "s" end)
              ),
              arg: $arg,
              variables: {
                "params": ($actual_params | tostring),
                "background": $path_obj.background
              },
